...
const SUBJECT_META = {
  ...
  cn:               { icon: "🌐", accent: "#10b981", label: "Computer Networks",    group: "cs" },
  ...
  infosys:          { icon: "🔵", accent: "#3b82f6", label: "Infosys",              group: "company" },
  wipro:            { icon: "🌿", accent: "#22c55e", label: "Wipro",                group: "company" },
  microsoft365:     { icon: "🪟", accent: "#0ea5e9", label: "Microsoft 365",        group: "company" },
...
const EXCLUDED_IDS = ["hr-questions", "interview-guides", "challenges", "quizgames", "learning"];

const DIFF_CONFIG = [
  { key: "easy",   label: "Easy",   icon: "🟢", desc: "Beginner-friendly concepts",    color: "#10b981", bg: "rgba(16,185,129,0.12)",  border: "rgba(16,185,129,0.35)" },
  { key: "medium", label: "Medium", icon: "🟡", desc: "Intermediate level questions",  color: "#f59e0b", bg: "rgba(245,158,11,0.12)",  border: "rgba(245,158,11,0.35)" },
  { key: "hard",   label: "Hard",   icon: "🔴", desc: "Advanced & tricky problems",    color: "#ef4444", bg: "rgba(239,68,68,0.12)",   border: "rgba(239,68,68,0.35)" },
  { key: "mixed",  label: "Mixed",  icon: "🎲", desc: "All difficulties shuffled",     color: "#a78bfa", bg: "rgba(167,139,250,0.12)", border: "rgba(167,139,250,0.35)" },
];
...
function getMeta(id) {
  return SUBJECT_META[id] || { icon: "📋", accent: "#6366f1", label: id.charAt(0).toUpperCase() + id.slice(1), group: "general" };
}
...
export default function TestPage() {
      ...
      .map(([folder, files]) => {
        ...
        return {
          ...
          files: hasDifficultySplit
            ? Object.fromEntries(Object.entries(files).filter(([k]) => DIFFICULTY_NAMES.includes(k)))
            : files,
        };
      })
  ...
  /* ─── Load questions ─── */
  const handleSelectCategory = useCallback(async (category, difficulty, mode) => {
    ...
    try {
      ...
      if (category.hasDifficultySplit) {
        if (difficulty === "mixed") {
          const paths = DIFFICULTY_NAMES.filter((d) => category.files[d]).map((d) => category.files[d]);
          const results = await Promise.all(paths.map((p) => allModules[p]()));
  ...
  const handleModeSelect = (mode) => {
    ...
    const diff = pendingCategory.hasDifficultySplit ? pendingDifficulty : "mixed";
    ...
    return (
      <div className="fixed inset-0 z-[100] w-full h-[100dvh] flex flex-col bg-[#0f172a] overflow-hidden">
        <div className="p-3 sm:p-4 flex flex-row justify-between items-center gap-3 border-b border-slate-700/60 bg-[#0a0f1a]/90 backdrop-blur sticky top-0 z-20">
          ...
          >
            ← <span className="hidden sm:inline">Back to Exams</span><span className="sm:hidden">Back</span>
          </button>
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0 text-right">
              <h2 className="text-sm sm:text-lg font-black text-white/90 truncate leading-tight">
                {allCategories.find(c => c.id === selectedCategory)?.displayName}
              </h2>
              <div className="flex items-center justify-end gap-1.5 mt-0.5 flex-wrap">
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: groupInfo.color, background: `${groupInfo.color}15`, border: `1px solid ${groupInfo.color}30` }}>
                  {groupInfo.icon} {groupInfo.label}
                </span>
                {diffLabel && (
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: diffLabel.color, background: diffLabel.bg, border: `1px solid ${diffLabel.border}` }}>
                    {diffLabel.icon} {diffLabel.label}
                  </span>
                )}
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${quizMode === "test" ? "bg-purple-500/20 text-purple-300 border border-purple-500/30" : "bg-blue-500/20 text-blue-300 border border-blue-500/30"}`}>
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 pb-24 pt-8 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            ...
            >
              ...
              <div className="px-6 pt-6 pb-4 border-b border-white/5 flex items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-2xl">{getMeta(pendingCategory.id).icon}</span>
                    <span className="text-white font-black text-lg leading-tight">{pendingCategory.displayName}</span>
                    {/* Group badge */}
                    <span
                      className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                      style={{
                        color: (GROUP_LABELS[pendingCategory.group] || {}).color || "#a78bfa",
                    ...
                    >
                      {(GROUP_LABELS[pendingCategory.group] || { icon: "📚" }).icon} {(GROUP_LABELS[pendingCategory.group] || { label: "General" }).label}
                    </span>
                  </div>
                ...
                <button onClick={closeModal} className="text-slate-500 hover:text-white transition-colors mt-1 shrink-0">
                  ...
                  {["difficulty", "mode"].map((step) => (
                    <div
                      ...
                      style={{
                        ...
                        background: modalStep === step ? getMeta(pendingCategory.id).accent : "rgba(255,255,255,0.15)",
                      }}
                    />
                  ))}
                </div>
                ...
                <div className="p-5 grid grid-cols-2 gap-3">
                  {DIFF_CONFIG.map((d) => (
                    ...
                    >
                      ...
                      <span className="absolute top-2 right-2 w-1.5 h-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: d.color }} />
                    </button>
                  ))}
                </div>
              )}
                  ...
                  {pendingDifficulty && (
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs text-slate-500">
                        {DIFF_CONFIG.find(d => d.key === pendingDifficulty)?.icon} {DIFF_CONFIG.find(d => d.key === pendingDifficulty)?.label} selected
                      </span>
                      {pendingCategory.hasDifficultySplit && (
                        <button onClick={() => setModalStep("difficulty")} className="text-[10px] text-slate-500 hover:text-white transition-colors underline underline-offset-2">
                          ← Change
                        </button>
                      )}
                    </div>
                  ...
                  {MODE_CONFIG.map((m) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-11 h-11 rounded-xl flex items-center justify-center text-xl shrink-0 transition-transform group-hover:scale-110" style={{ background: `${m.color}20` }}>
                          {m.icon}
                        </div>
                      ...
                      <span className="text-slate-500 group-hover:translate-x-1 transition-transform" style={{ color: m.color }}>→</span>
                    </button>
                  ))}
                </div>
              )}
              ...
              <div className="px-5 pb-5">
                <button onClick={closeModal} className="w-full py-2.5 text-slate-500 hover:text-slate-300 text-sm font-semibold transition-colors rounded-xl hover:bg-white/5">
                  Cancel
                </button>
              </div>
            </motion.div>
      ...
      <div className="mb-8 animate-fade-in-up">
        <p className="text-xs font-semibold uppercase tracking-[0.35em] text-[#a78bfa]/90">Exam Center</p>
        <h1 className="mt-1 text-4xl font-black text-white sm:text-5xl">All Exams</h1>
        <p className="mt-2 text-sm text-slate-400">Select a subject · choose difficulty · choose mode</p>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {Object.entries(groupedVisible).map(([groupKey, cats]) => {
            const grp = GROUP_LABELS[groupKey] || { label: groupKey, icon: "📂", color: "#6366f1" };
            return (
              <div key={groupKey} className="mb-10">
                ...
                {activeGroup === "all" && (
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <div className="flex-1 h-px" style={{ background: `linear-gradient(90deg, ${grp.color}30, transparent)` }} />
                  </div>
                )}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          ...
                          style={{ background: `radial-gradient(circle at 30% 20%, ${meta.accent}12, transparent 65%)` }}
                        />
                        ...
                        <div className="relative flex items-start justify-between">
                          ...
                          {category.hasDifficultySplit ? (
                            <div className="flex gap-1" title="Has Easy · Medium · Hard levels">
                              {DIFFICULTY_NAMES.map((d) => {
                                const dc = DIFF_CONFIG.find((x) => x.key === d);
                                return <span key={d} className="text-sm">{dc.icon}</span>;
                              })}
                            </div>
                          ) : (
                            <span
                              className="text-[10px] font-bold px-2 py-1 rounded-lg"
                              style={{ color: grp.color, background: `${grp.color}15`, border: `1px solid ${grp.color}25` }}
                            >
                              {grp.icon}
                            </span>
                          )}
                        ...
                        <div className="relative mt-4 flex-1">
                          <h3 className="text-lg font-black tracking-tight text-white">{category.displayName}</h3>
                          <p className="mt-1 text-xs text-slate-500 leading-relaxed">
                            {category.hasDifficultySplit ? "Easy · Medium · Hard · Mixed" : "Question dataset"}
                          </p>
                        </div>
